...
Sources may be imported, searched, viewed, and used to propose context suggestions, and relevant excerpts may be retrieved into chat context. None of these paths write `memory_entries`. The approval boundary is unchanged: only explicit `approveDraft` creates approved context. The model may read selected source excerpts and approved context to generate a response; it may not approve context, execute tools, browse, access accounts, schedule work, or write approved context directly. File import reads only the file the user explicitly selects (read client-side in the browser and posted as text); it never crawls or reads arbitrary filesystem paths, and a per-file size limit (~1 MB of text) applies.
