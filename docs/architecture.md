...
ContextCrate (internal/codename HERmes) is a local context generator for LLM-ready context packs ("crates"). It has a localhost-only web UI whose default home is the **Crates** surface, a small SQLite-backed service layer, approval-first context suggestions, a source library, a crate (LLM context-pack) export flow, an optional secondary **Capture** chat surface backed by a chat provider layer with an optional Claude API mode, and an advanced CLI. The browser UI is local-only; it is not an external web app and does not add automation capabilities.

The product model has four nouns: **Sources** (raw imported material), **Approved context** (durable, human-approved context — the only thing used for chat, search, reflection, and crate generation), **Crates** (saved Markdown context-pack snapshots for LLM handoff, which are export artifacts and not approved context), and **Capture chat** (an optional secondary surface that can suggest possible context for review). External LLMs are used by copying a crate out; the app itself takes no external action.
...
The Crates page (`renderMemoryPackPage`) is the default home at `GET /`, with `GET /memory-pack` kept as a stable alias so existing routes/tests keep working. It is titled **Context crates** and explains that "A crate is a saved Markdown context pack you can copy or download for an LLM." It lets James search active approved context, choose checkboxes, provide a project title, optional current next step, and optional **Settled decisions / things not to reopen** notes, then posts to `POST /memory-pack/export` through the same local UI POST guard as the rest of the app. The helper text says: "Add decisions that are already settled, so a coding assistant does not waste time suggesting them again." **Create crate** renders the generated Markdown directly on the page as **Generated crate**, with **Copy crate** as the primary action and **Download Markdown** as the secondary file path. A separate `POST /memory-pack/save` stores the generated Markdown as a `context_packs` snapshot so it can be viewed, copied, and downloaded later from **Saved crates**. Internal route paths, table names, and function names keep the `memory-pack`/`contextPack` naming; only the visible noun changed to "crate".
...
Sources may be imported from an explicitly chosen file or pasted Markdown/text, renamed, deleted, searched, viewed, and used to propose context suggestions, and relevant excerpts may be retrieved into chat context. None of these paths write `memory_entries`. Pasted source import reuses `importSource` with a synthetic `.md` filename, creating only a raw source and chunks. Renaming changes only the source title. Deleting a source removes the raw `sources` row and its `source_chunks`, but approved context already created from it remains in `memory_entries`; pending drafts are not automatically cleaned up. The approval boundary is unchanged: only explicit `approveDraft` creates approved context. The model may read selected source excerpts and approved context to generate a response; it may not approve context, execute tools, browse, access accounts, schedule work, or write approved context directly. File import reads only the file the user explicitly selects (read client-side in the browser and posted as text); paste import reads only the text the user pasted. It never crawls or reads arbitrary filesystem paths, and a per-source size limit (~1 MB of text) applies.
...
As of the ContextCrate framing pass, the browser UI uses context-generator language and hides developer/internal terms. The UI says "ContextCrate", "context suggestion", "This seems useful as approved context.", "Save for review", "Saved for review", "Review context suggestions", "Approve context", "Manage context", and "Context crates". As of the crates recenter pass (v0.4.10) the default home is the **Crates** page, the main saved/exported artifact is a **crate** ("Create crate", "Generated crate", "Copy crate", "Save crate", "Saved crates"), and the chat surface is relabeled **Capture** ("Capture chat", "Chat can help capture possible context for review.", "Nothing becomes approved context until you approve it.") at the secondary route `/capture`. "Crate" is the primary noun; "LLM context pack" is kept as secondary explanatory wording. The pages no longer expose "draft", database paths, or table names.
...
The default page (`GET /`) is the **Crates** surface: it generates, copies, downloads, and saves Markdown context packs from selected active approved context, and lists **Saved crates**. Its top navigation links to **Crates**, **Sources**, **Manage context** (`/memories`), **Review context**, and **Capture** (`/capture`), with **Diagnostics** demoted to a small footer link. The **Capture** page (`renderPage`, `GET /capture`) holds the chat surface: app name, a subtle conversation-mode label ("Capture chat · Mode: ..."), chat history, message input, assistant responses, approved-context sources, organic context suggestions, save-for-review, Add context, and Review context suggestions; all chat/draft POST handlers re-render this page, so chat stays fully functional. Approved-context search, edit, and retire live on the Manage context page. Source rename/delete and pasted-source import controls live on the Sources page. Technical diagnostics, database path, table status, deterministic reflection, and local JSON export live behind `/system`, reached only via the footer "Diagnostics" link. The UI opens in Local mode with no API key required; Claude mode is optional and only affects chat-response wording when configured.
...
Saved context packs are stored in `context_packs` as Markdown export snapshots with title, creation timestamp, Markdown body, and optional filename/export path. Saving a pack does not create `memory_entries`, approve `memory_drafts`, or modify `sources`.
